...
class Reservoir_Outlet
{
  public:
    ...
    {
      if (storage_struct.current_storage_meters <= activation_threshold_meters)
        return 0.0;  
	  
      return a * std::pow((storage_struct.current_storage_meters - activation_threshold_meters)
...
class Nonlinear_Reservoir
{
  public:
    ...
    //Nonlinear_Reservoir(double minimum_storage_meters, double maximum_storage_meters, double current_storage_meters) : outlets()
    //NO OUTLET
	Nonlinear_Reservoir(double minimum_storage_meters, double maximum_storage_meters, double current_storage_meters)
    {

      parameters.minimum_storage_meters = minimum_storage_meters;
      parameters.maximum_storage_meters = maximum_storage_meters;
      state.current_storage_meters = current_storage_meters;
    }

	//ONE OUTLET
    Nonlinear_Reservoir(double minimum_storage_meters, double maximum_storage_meters, double current_storage_meters, double a, double b, double activation_threshold_meters) : Nonlinear_Reservoir(minimum_storage_meters, maximum_storage_meters, current_storage_meters)
    {
      this->outlets.push_back(Reservoir_Outlet(a, b, activation_threshold_meters));
    }

    //MULTIPLE OUTLETS
    Nonlinear_Reservoir(double minimum_storage_meters, double maximum_storage_meters, double current_storage_meters, std::vector <Reservoir_Outlet> &outlets) : Nonlinear_Reservoir(minimum_storage_meters, maximum_storage_meters, current_storage_meters)
    {
...
//NOT GOOD TO UPDATE STATE AND FLUX CALCULATORS BACK TO THE CALLING FUNCTION. LET CALLING UPDATE ITSELF.
    ...
    {	
      state.current_storage_meters = in_flux_meters_per_second * delta_time_seconds;
      ...
      {
        //Update storage  
        state.current_storage_height_meters -= outlet.discharge_meters_per_second(parameters, state) * delta_time_seconds;
      }

      //TODO: Return extra storage to caller
      //Is this being called with an in_flux rate or storage as inputs?
      //		
      //if (state.current_storage_meters > state.maximum_storage_meters)
      //  in_flux_meters_per_second += (maximum_storage_meters - current_storage_meters) / dt
      //  current_storage_meters = maximum_storage_meters
    }

    double get_storage_meters()
    {
      return state.current_storage_meters;
    }
